...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBashCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBashCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBashCoinAmount = strAnonymizeBashCoinAmount.remove(strAnonymizeBashCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBashCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBashCoinAmount*COIN) nMaxToAnonymize = nAnonymizeBashCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBashCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBashCoinAmount));
        strAnonymizeBashCoinAmount = strAnonymizeBashCoinAmount.remove(strAnonymizeBashCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBashCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
