...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBashCoinAmount"))
        settings.setValue("nAnonymizeBashCoinAmount", 1000);
    nAnonymizeBashCoinAmount = settings.value("nAnonymizeBashCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBashCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBashCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBashCoinAmount:
            return QVariant(nAnonymizeBashCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBashCoinAmount:
            nAnonymizeBashCoinAmount = value.toInt();
            settings.setValue("nAnonymizeBashCoinAmount", nAnonymizeBashCoinAmount);
            emit AnonymizeBashCoinAmountChanged(nAnonymizeBashCoinAmount);
            break;
        default:
            break;
        }
